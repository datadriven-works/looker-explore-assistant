...
import contextPrompts from '../utils/contextPrompts'
    ...
    async (prompt: string) => {
      const contents = `
        ...
        Instructions:
          ...
          - you must take into account the following key aspects around Looker Explore:

          ${contextPrompts.map((prompt) => prompt).join('\n')}
